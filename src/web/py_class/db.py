...
class DB(object):
    ...
    def create_user(self, name, email=None, passwordN=None, passwordM=None, google_id=None, facebook_id=None, twitter_id=None):
        ...
        secure_passN = hashlib.sha256((salt+passwordN).encode('UTF-8')).hexdigest() if passwordN else None
        secure_passM = hashlib.sha256((salt+passwordM).encode('UTF-8')).hexdigest() if passwordM else None

        data = {"email": email, "name": name, "salt": salt, "passwordN": secure_passN,
                "passwordM": secure_passM, "user_id": user_id, "google_id": google_id,
    ...
    def get_user(self, name=None, email=None, password=None, id_type="user", user_id=None):
        ...
        if name:
            ...
            if _user:
                ...
                if not password or _user.get("passwordN")==secure_pass:
                    return _user
        ...
        # If no name provided, lookup user by email
            ...
            if _user:
                ...
                salt = _user.get("salt")
                secure_pass = hashlib.sha256((salt+password).encode('UTF-8')).hexdigest()
                if not password or _user.get("passwordM")==secure_pass:
                    return _user
